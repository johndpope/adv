...
X, y = datasets.make_classification(n_samples=10000, n_features=784,
                                    n_informative=2, n_redundant=2)

train_samples = 8000  # Samples used for training the models
...
for clf, name in [(mlp, 'MLP'),
                  ...
                  (resnet, 'ResNet')]:
    if name.lower() == "mlp":
        # clf.fit(X_train.reshape(-1, 784),
        #         np_utils.to_categorical(y_train, 10))
        prob_pos = np.max(clf.predict(X_test.reshape(-1, 784)), axis=1)
    else:
        # clf.fit(X_train.reshape(-1, 28, 28, 1),
        #         np_utils.to_categorical(y_train, 10))
        prob_pos = np.max(clf.predict(X_test.reshape(-1, 28, 28, 1)), axis=1)

    # if hasattr(clf,resnet "predict_proba"):
    # prob_pos = np.max(clf.predict(X_test), axis=1)
